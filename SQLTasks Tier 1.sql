...
FROM Members AS m
INNER JOIN Facilities AS f ON memid
WHERE memid
IN (
...
SELECT CONCAT(m.firstname || ' ' || m.surname) AS fullname, name AS facility
CASE WHEN b.memid = 0 
...
SELECT DISTINCT(name), SUM(slots)
        FROM Bookings
        ...
        WHERE slots >= 1
        AND Bookings.memid IS NOT NULL
        GROUP BY Bookings.facid
/* Q13: Find the facilities usage by month, but not guests */

SELECT DISTINCT(name), SUM(slots), strftime('%m', starttime)
        FROM Bookings
        LEFT JOIN Facilities
        ON Bookings.facid = Facilities.facid
        LEFT JOIN Members 
        ...
        WHERE slots >= 1
        AND Bookings.memid IS NOT NULL
        GROUP BY Bookings.facid, strftime('%m', starttime)